<?php
/**
 * One list of bundles, and whatever C&F can do with them at this stage.
 *
 * Every bundle opens to show who is inside it and what each of them is owed,
 * because that is what C&F is actually paying — the total on the outside is
  ...
  <div class="panel__body">
    <?php foreach ($rfBundles as $rfBundle): ?>
      <?php
        ...
        $cfMissing = $rfBankable ? [] : [$rfParty['full_name'] ?? 'the distributor'];

        foreach ($rfChildren as $rfChild) {
            if (!voucher_has_bank($rfChild)) {
                $rfChildParty = voucher_party($rfChild);
                $cfMissing[]  = $rfChildParty['full_name'] ?? 'a dealer';
            }
        }
      ?>
        ...
        <?php if ($cfMissing): ?>
          <p class="direct-sale__notice">
            ...
            <span>
              ...
              <?= e(implode(', ', array_unique($cfMissing))) ?>
              <?= count(array_unique($cfMissing)) === 1 ? 'has' : 'have' ?>
        ...
        <div class="voucher__actions">
          ...
          <?php elseif ($rfMode === 'pay'): ?>
            <form method="post" class="decide__reason"
              ...
              <button type="submit" class="btn btn--primary" <?= $cfMissing ? 'disabled' : '' ?>>
