<?php
/**
 * Applicant sign-in: email address, then a one-time code sent to it.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'request') {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $error = 'Enter the email address you used on your application.';
        } else {
            $error = issue_otp($email);
...
<section class="section portal-auth">
  <div class="container-x">
    <div class="portal-card">
      <p class="eyebrow eyebrow--rule">Applicant sign in</p>
      <h1>Track your application.</h1>
      <p class="u-sub portal-card__lead">
        No password. Enter the email address you applied with and we send a one-time code.
      </p>
      ...
      <?php if (!mail_configured()): ?>
        <p class="portal-alert portal-alert--warn">
          ...
          Call <a href="tel:+919725154186">+91 97251 54186</a> and we will look your application up.
        </p>
      <?php endif; ?>
